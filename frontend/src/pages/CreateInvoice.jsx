...
export default function CreateInvoice() {
  ...
  reader.onload = (e) => {
    setInvoice((inv) =>
      inv ? { ...inv, [`${kind}DataUrl`]: e.target.result } : inv
    );
  };
  reader.readAsDataURL(file);
}
    ...
    try {
      ...
      const endpoint =
        isEditing && invoice.id
          ? `${API_BASE}/api/invoices/${invoice.id}`  // ✅ FIXED
    : `${API_BASE}/api/invoices`;               // ✅ FIXED
...
// ✅ Convert base64 → File
const logoFile = dataURLtoFile(invoice.logoDataUrl, "logo.png");
const stampFile = dataURLtoFile(invoice.stampDataUrl, "stamp.png");
const signatureFile = dataURLtoFile(invoice.signatureDataUrl, "signature.png");
...
const cleanedData = { ...prepared };

// ❌ remove base64 fields (VERY IMPORTANT)
delete cleanedData.logoDataUrl;
delete cleanedData.stampDataUrl;
delete cleanedData.signatureDataUrl;
